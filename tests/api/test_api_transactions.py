...
def _seed_transaction_records():
    ...
    db.create_owned_stock(seller_id, "AAPL", 20)

    cursor.execute("DELETE FROM Transactions;")
    cursor.execute(
        """INSERT INTO Transactions (bidder_id, bid_price, asker_id, ask_price, vol, ticker, time_stamp, transaction_price)\n        VALUES (?, ?, ?, ?, ?, ?, ?, ?);""",
        (buyer_id, 100.0, seller_id, 100.0, 1, "AAPL", "2025-01-01 12:00:00", 100.0),
    )
    cursor.execute(
        """INSERT INTO Transactions (bidder_id, bid_price, asker_id, ask_price, vol, ticker, time_stamp, transaction_price)\n        VALUES (?, ?, ?, ?, ?, ?, ?, ?);""",
        (buyer_id, 101.0, seller_id, 101.0, 2, "AAPL", "2025-01-02 12:00:00", 101.0),
    )
    connection.commit()
    connection.close()
...
def test_transactions_endpoint_happy_path_returns_only_public_fields(
    ...
    response = api_client.get(
        "/api/transactions", params={"ticker": "AAPL", "limit": 20}
    )
...
def test_transactions_endpoint_rejects_invalid_limit(api_client, database_snapshot):
    response = api_client.get(
        "/api/transactions", params={"ticker": "AAPL", "limit": 0}
    )
