...
def test_place_order_invalid_limit_price():
    client = TestClient(app)
    response = client.post(
        "/api/place_order",
        json={
...
def test_market_order_invalid_volume():
    client = TestClient(app)
    response = client.post(
        "/api/market_order",
        json={
...
def test_cancel_order_happy_path_and_structure():
    client = TestClient(app)
    ...
    response = client.post("/api/cancel_order", json={"order_id": order_id})
    assert response.status_code == 200
    body = response.json()
...
def test_cancel_order_not_found():
    client = TestClient(app)
    response = client.post("/api/cancel_order", json={"order_id": 999_999})
    assert response.status_code == 404
    assert response.json()["detail"] == "Order 999999 does not exist."


def test_edit_order_happy_path_and_structure():
    client = TestClient(app)
    order_response = client.post(
        "/api/place_order",
        json={
...
def test_edit_order_invalid_price_or_volume():
    client = TestClient(app)
    order_response = client.post(
        "/api/place_order",
        json={
    ...
    response = client.post(
        "/api/edit_order", json={"order_id": order_id, "price": -1.0, "volume": 1}
    )
    assert response.status_code == 400
